;(function(window) {
	...
	PieceMaker.prototype._init = function() {
		...
		// Container sizes.
		this.dimensions = {width: window.innerWidth/1.5, height: window.innerHeight/1.5}; //{width:this.el.offsetWidth, height:this.el.offsetHeight};
	...
	PieceMaker.prototype._initEvents = function() {
		const self = this,
			  ...
			  onMouseMoveFn = function(ev) {
				if (self.el.parentElement.classList.contains("section--is-active")){
			  ...
			  debounceResizeFn = debounce(function() {
				...
				self.dimensions = {width: elBounds.width, height: elBounds.height/3};
	...
	function init() {
		imagesLoaded(DOM.body, { background: true }, function() {
			...
			pm.loopFx();
			// Glitch effect on some elements (contact and coder link) in the page.
			gfx = new GlitchFx(DOM.glitchElems);
			// Split the contact and code menu items into spans/letters.
			wordsToLetters();
			// Init/Bind events
			initEvents();
		});
	...
	function switchToCode() {
		...
		// Animate the contact, menu ctrl and menu items out and show the code mode version of these elements.
		// Contact:
	...
	function switchToDesign() {
		...
		// Animate contact, menu ctrl and menu items out and show the design mode version of these elements.
		// Contact:
